...
def main():
    ...
    root.bind('<f>', lambda event: send_up(mqtt_client))

    down_button = ttk.Button(main_frame, text="Down")
    down_button.grid(row=6, column=0)
    down_button['command'] = lambda: send_down(mqtt_client)
    root.bind('<r>', lambda event: send_down(mqtt_client))
...
def send_forward(mqtt_client, lspeedentry, rspeedentry):
    print("Go Forward")
    lspeed = lspeedentry.get()
    lspeed = int(lspeed)
    rspeed = rspeedentry.get()
    rspeed = int(rspeed)
    mqtt_client.send_message("goforward", [lspeed, rspeed])


def send_back(mqtt_client, lspeedentry, rspeedentry):
    print("Go Backwards")
    lspeed = lspeedentry.get()
    lspeed = int(lspeed)
    rspeed = rspeedentry.get()
    rspeed = int(rspeed)
    mqtt_client.send_message("goback", [lspeed, rspeed])


def send_left(mqtt_client, lspeedentry, rspeedentry):
    print("Back Up")
    print("Spin Left")
    lspeed = lspeedentry.get()
    lspeed = int(lspeed)
