...
def main():
    ...
    forward_button['command'] = lambda: send_forward(mqtt_client, int(left_speed_entry.get()),
                                                     int(right_speed_entry.get()))
    root.bind('<Up>',
              lambda event: send_forward(mqtt_client, int(left_speed_entry.get()), int(right_speed_entry.get())))
