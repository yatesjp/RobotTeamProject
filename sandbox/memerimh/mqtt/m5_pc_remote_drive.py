...
Authors: David Fisher and Mattias Memering.
...
def main():
    # TDO: 2. Setup an mqtt_client.  Notice that since you don't need to receive any messages you do NOT need to have
    ...
    forward_button['command'] = lambda: send_forward(mqtt_client, int(left_speed_entry.get()), int(right_speed_entry.get()))
    root.bind('<Up>', lambda event: send_forward(mqtt_client, int(left_speed_entry.get()), int(right_speed_entry.get())))

    left_button = ttk.Button(main_frame, text="Left")
    left_button.grid(row=3, column=0)
    # left_button and '<Left>' key
    left_button['command'] = lambda: send_left(mqtt_client, int(left_speed_entry.get()), int(right_speed_entry.get()))
    root.bind('<Left>', lambda event: send_left(mqtt_client, int(left_speed_entry.get()), int(right_speed_entry.get())))
    ...
    root.bind('<Right>', lambda event: send_right(mqtt_client, int(left_speed_entry.get()), int(right_speed_entry.get())))
